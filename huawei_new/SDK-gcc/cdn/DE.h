...
//            }
            ...
            for (int i = tzkw->head[tzkw->src]; i != -1; i = tzkw->edge[i].next) {
                ...
                if (e.cap != e.pre_cap) {
                    ...
                    video_list[e.v] = vt+1;
                    nodeCost[e.v] = frand(0.1)*(global->buildCost[e.v] + global->videoCost[vt].cost) * 1.0 / (e.pre_cap - e.cap);
                } else
